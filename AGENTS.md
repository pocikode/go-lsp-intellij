...
- Editor features (diagnostics, completion, hover, navigation, references, code actions, formatting, folding, inlay hints) come from the platform LSP client and `gopls`; which ones exist depends on the IDE version. Debugging, coverage, automatic `gopls` download, and Go-version selection are roadmap work. Format-on-save is implemented through local `gofmt`, with optional `goimports` import organization.
- The Go test runner is implemented: gutter run arrows on `*_test.go`, a "Go Test" run configuration, and the platform test tree fed from `go test -json`, with subtests nested and rerun-failed. It uses no LSP at all - see the Architecture section for why that is deliberate.
